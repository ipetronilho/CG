...
{   
	...
	//----------------------------------------- Chaleira
	glGenTextures(1, &texture[2]);											// cria o id da textura
	glBindTexture(GL_TEXTURE_2D, texture[2]);								// cria o objeto textura, associando ao id
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_DECAL);				// filtros + repetições
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
	imag.LoadBmpFile("chaleira.bmp");
	glTexImage2D(GL_TEXTURE_2D, 0, 3, 										// constrói a textura
				imag.GetNumCols(),
				imag.GetNumRows(), 0, GL_RGB, GL_UNSIGNED_BYTE,
				imag.ImageData());

	//----------------------------------------- Parede z=0
	glGenTextures(1, &texture[3]);											// cria o id da textura
	glBindTexture(GL_TEXTURE_2D, texture[3]);								// cria o objeto textura, associando ao id
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_DECAL);				// filtros + repetições
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	...
	//----------------------------------------- Parede x=0
	glGenTextures(1, &texture[4]);											// cria o id da textura
	glBindTexture(GL_TEXTURE_2D, texture[4]);								// cria o objeto textura, associando ao id
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_DECAL);				// filtros + repetições
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	...
	int i;
	for (i=0;i<5;i++) {
			glGenTextures(1, &horseTexture[i]);											// cria o id da textura
			glBindTexture(GL_TEXTURE_2D, horseTexture[i]);								// cria o objeto textura, associando ao id
			glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_DECAL);					// filtros + repetições
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
	...
	//----------------------------------------- Quadro
	//--------------------------- Imagem01
	/*glGenTextures(1, &texture[5]);											// cria o id da textura
	glBindTexture(GL_TEXTURE_2D, texture[5]);								// cria o objeto textura, associando ao id
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_DECAL);				// filtros + repetições
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
...
void drawScene(){
	...
	glPushMatrix();
		glTranslatef( mesaP[0], mesa+mesa/4, mesaP[2]);
	...
	//glBindTexture(GL_TEXTURE_2D, horseTexture[0]);
	glBindTexture(GL_TEXTURE_2D, horseTexture[x%5]);
	x+=1;
	glPushMatrix();
		glBegin(GL_QUADS);
			glTexCoord2f(0.0f,0.0f); glVertex3i( 5,  5, 1 );	// idealmente z=0, mas para não se sobrepor à parede z=0 considero que z=1
			glTexCoord2f(1.0f,0.0f); glVertex3i( 10, 5, 1 ); 
			glTexCoord2f(1.0f,1.0f); glVertex3i( 10, 10, 1); 
			glTexCoord2f(0.0f,1.0f); glVertex3i( 5,  10,  1); 
		glEnd();
	glPopMatrix();
	glDisable(GL_TEXTURE_2D);
